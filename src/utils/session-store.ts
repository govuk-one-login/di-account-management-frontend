import { DynamoDBClient, QueryCommand, DynamoDBClientConfig } from "@aws-sdk/client-dynamodb";
...
async function getSessions(subjectId: string): Promise<string[]> {
    ...
    try {
        ...
        logger.error(`session-store - failed to get sessions: ${JSON.stringify(error)}`);
    }
    return [];
}

export async function destroyUserSessions(subjectId: string, sessionStore: Store): Promise<void> {
    const sessionIds = await getSessions(subjectId);
    const destroySessions = sessionIds.map((sessionId) => Promise.resolve(sessionStore.destroy(sessionId)));
    try {
        const results = await Promise.allSettled(destroySessions);
        if (results.some(result => result.status === "rejected")) {
            logger.warn(`session-store - failed to delete session(s): ${results.filter(result => result.status === "rejected").length} out of ${results.length} failed`);
        }
    } catch (error: any) {
        logger.error(`session-store - failed to delete session(s): ${JSON.stringify(error)}`);
    }
}
