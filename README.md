...
Mention that running tests will start app and test server which can take some time and will be killed when the tests end. If doing a prolonged period of testing it will be quicker start the app and test server separately using `cd end-to-end-tests && npm run build-and-run-app` and `cd end-to-end-tests && npm run start-test-server` respectively.
