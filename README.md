...
docker build -t frontend-integration-tests .
docker run -t frontend-integration-tests # or docker run -t -v `pwd`/snapshots:/snapshots -v `pwd`/test-results:/test-results --network="host" frontend-integration-tests if running the tests against localhost
```
...
TODO
mention that you can use the Playwright extension for VS Code but to pick up changes to test files you need to run `npm run watch` and also that the tests in the Test Explorer are not linked to the step files but to the test files generated by playwright-bdd which is not super helpful.
